...
const defaultSkills = [
  {
    ...
    name: '信源布局差异分析',
    description: '基于发布信源库和全网采集信源，分析信源布局缺口、高频引用信源和真实引用差异。',
    type: 'data',
    creator: '范范',
    ...
    inputSchema: [
      ...
      { key: 'ownedSources', label: '发布信源库概况', type: 'textarea', placeholder: '可填写官网、旗舰店、知乎专栏、小红书账号等已发布或已维护信源' },
...
const runActiveSkill = async () => {
  ...
  if (activeSkill.value.runMode === 'local') {
    ...
    runResult.value = [
      ...
      '- 对“已收录但引用次数低”的发布信源，补充结构化标题、摘要、核心问题答案和可验证数据。',
      '- 对“未收录”的发布信源，优先处理可被搜索引擎抓取的页面、站点地图、页面可访问性和内容更新频率。',
      '',
      '2. 哪些全网高频信源还没有布局？',
      '- 对比全网采集信源的引用次数 TOP 列表，重点检查知乎、小红书、京东/天猫、垂直媒体、百科/问答平台是否已有对应自有或合作内容。',
      '- 如果竞品测评页、低质排行榜页、论坛帖被多模型引用，但发布信源库没有同类内容，需要补充同主题的正向解释信源。',
      '- 对高频但不可控的第三方信源，标记为“需替代布局”，用更权威的官方/合作/媒体内容承接同一批问题。',
      '',
      '3. 发布信源和全网真实引用之间差在哪里？',
      '- 看覆盖差：发布信源是否出现在大模型回答引用里；未被引用说明收录或权威不足。',
      '- 看问题差：发布信源希望影响的问题，是否就是全网真实引用最多的问题；不一致说明内容主题没有对齐问题配置。',
      '- 看模型差：同一信源是否只被少数模型引用；如果只覆盖豆包/DeepSeek，需检查其他模型依赖的平台和语料来源。',
      '- 看平台差：全网真实引用集中在哪些平台，发布信源库是否覆盖同类平台。'
    ].join('\n')
    running.value = false
    return
  }
